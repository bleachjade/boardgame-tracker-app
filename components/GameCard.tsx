    ...
    >
      ...
      <div className="p-2 sm:p-5 flex-1 flex flex-col justify-start md:justify-between">
        <div>
          ...
          <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mb-4">{game.year || t('gameCard.unknownYear')}</p>
          <div className="flex justify-between text-xs font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-900 p-2.5 rounded-lg md:mb-4">
            <span>{game.minPlayers || '?'}-{game.maxPlayers || '?'} {t('gameCard.players')}</span>
            <span>{game.playTime || '?'} {t('gameCard.mins')}</span>
          </div>
        </div>

        {!isBulkMode && (
          <div className="space-y-2">
            <button onClick={(e) => onLogScore(e, game)} className="w-full flex items-center justify-center gap-2 py-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 font-bold rounded-xl text-sm border border-indigo-100 dark:border-indigo-800 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition shadow-sm mt-[8px] md:mt-0"><Trophy size={16} /> {t('gameCard.logScore')}</button>
            {game.userId === userUid ? (
              <button onClick={(e) => onAssign(e, game)} className="w-full flex items-center justify-center gap-2 py-2 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 font-bold rounded-xl text-sm hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition"><FolderPlus size={16} /> {t('gameCard.assignLists')}</button>
            ) : (
              <div className="w-full flex items-center justify-center gap-2 py-2 border border-transparent text-slate-500 dark:text-slate-400 font-medium rounded-xl text-xs bg-slate-50 dark:bg-slate-900/50">{t('gameCard.sharedByFriend')}</div>
            )}
          </div>
        )}
      </div>
      ...
      {!isBulkMode && expansions && expansions.length > 0 && (
        <div className="bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700 p-3 mt-auto">
          <h4 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-1 mb-2"><Puzzle size={12}/> {t('gameCard.expansions')} ({expansions.length})</h4>
          <div className="space-y-2">
            {expansions.map(exp => (
