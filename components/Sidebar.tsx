            ...
            <div className="space-y-1">
              {customGroups.map(group => (
                <div key={group.id} className="relative group/btn">
                  <button onClick={() => selectGroupMobile(group)} className={`w-full text-left p-2.5 pr-10 rounded-xl flex items-center gap-3 transition-colors ${activeGroup?.id === group.id && currentView === "library" ? 'bg-indigo-600 text-white font-bold shadow-md' : 'text-slate-700 dark:text-slate-300 font-medium hover:bg-indigo-50 dark:hover:bg-indigo-900/30'}`}><Users size={18} className={activeGroup?.id === group.id && currentView === "library" ? "text-indigo-200" : "text-indigo-400"} /> <span className="truncate">{group.name}</span></button>
