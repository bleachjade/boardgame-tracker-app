...
import { Trophy, X, Calculator, History, Calendar, Plus, Users, Image as ImageIcon, CheckCircle, XCircle, Trash2, Mic, MicOff, UserCheck, Circle } from "lucide-react";
...
export function ScoresModal({ game, onClose }: { game: any; onClose: () => void }) {
  ...
  const appendMath = (index: number, operator: string) => {
    const current = players[index].scoreInput;
    updatePlayer(index, "scoreInput", current + operator);
  };
  ...
  const handleSavePlay = async () => {
    ...
    const finalScores = players.map((p, idx) => ({
      ...
      rawExpression: playMode === "coop" ? "" : playMode === "winner" ? (winnerIndex === idx ? "Win" : "Loss") : p.scoreInput
    }));
          ...
          {activeTab === "log" ? (
            <div className="space-y-5">
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{playMode !== "score" ? t('scores.playersInvolved') : t('scores.playerScores')}</h3>
                ...
                {players.map((player, idx) => {
                  ...
                  return (
                    <div key={idx} className="bg-white dark:bg-slate-800 p-3 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm space-y-2.5 animate-in fade-in slide-in-from-bottom-2 duration-200">
                      ...
                      {playMode === "score" && (
                        <div className="flex items-center justify-end gap-1 pt-1 border-t border-slate-100 dark:border-slate-700/50 flex-wrap">
                          {player.scoreInput.includes("+") || player.scoreInput.includes("*") || player.scoreInput.includes("-") ? (
                            <span className="text-[10px] font-bold text-slate-400 mr-auto self-center truncate max-w-[100px]">{t('scores.eq')} {player.scoreInput}</span>
                          ...
                          <div key={pIdx} className="flex justify-between items-center text-sm font-semibold">
                            ...
                            <div className="text-right">
                              <span className={`font-black ${p.rawExpression === "Win" ? "text-amber-600 dark:text-amber-400" : "text-slate-400 dark:text-slate-500"}`}>
                                {p.rawExpression === "Win" ? t('scores.winner') : t('scores.loser')}
                              </span>
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="space-y-1.5">
                        {sortedPlayers.map((p: any, pIdx: number) => (
                          <div key={pIdx} className="flex justify-between items-center text-sm font-semibold">
                            ...
                            <div className="text-right">
                              <span className="font-black text-slate-900 dark:text-white">{p.score} {t('scores.pts')}</span>
